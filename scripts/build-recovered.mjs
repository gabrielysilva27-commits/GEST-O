...
for (const file of ["gerot-delivery-data.js","gerot-delivery-engine.js","gerot-delivery-model.js","gerot-delivery-editor.js","gerot-delivery-launcher.js","gerot-entrega-standalone.js"]) entries.push({route:'/assets/js/'+file,kind:'text',contentType:'application/javascript; charset=utf-8',body:''});
...
users.splice(0,users.length,...browserUsers.map(({id,username,name,title,department})=>({id,username,name,title,department})));users.push({id:1,username:'Gabriely',name:'Gabriely',title:'Gabriely'});
...
for (const file of ["gerot-delivery-data.js","gerot-delivery-engine.js","gerot-delivery-model.js"]) await fs.copyFile('assets/js/'+file,'dist/server/'+file);
await fs.writeFile('dist/server/index.js','import {hydrateDeliveryArea,applyDeliveryCells} from "./gerot-delivery-model.js";\nimport {listDtos,dtoItem} from "./dto-items.js";\nimport {anomalyRequest} from "./anomaly-items.js";\nconst DTO_USERS='+JSON.stringify(users)+';\n'+runtime);
