...
users.push({id:1,username:'Gabriely'});
...
runtime=runtime.replace(marker,'    const dtoResponse = await dtoItem(request,this.state.storage,claim,DTO_USERS); if(dtoResponse)return dtoResponse;\n'+marker);
runtime=runtime.replace('if (pathname === "/api/session"','if (pathname.startsWith("/api/dto-applications/") || pathname === "/api/session"');
