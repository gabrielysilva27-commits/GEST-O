...
async function withCentralImport(state, data) { const overlay = (await state.storage.get("centralImportOverlay")) || [], live = (await state.storage.get("liveActions")) || [], deleted = new Set((await state.storage.get("deletedLiveActionIds")) || []), liveNotifications = (await state.storage.get("liveNotifications")) || []; if (!data) return data; const base = [...(Array.isArray(data.actionPlans) ? data.actionPlans : []), ...overlay].filter((item) => !deleted.has(Number(item?.id))); const byId = new Map(base.map((item) => [Number(item.id), item])); live.filter((item) => !deleted.has(Number(item?.id))).forEach((item) => byId.set(Number(item.id), item)); const notifications = [...(Array.isArray(data.notifications) ? data.notifications : []), ...liveNotifications].filter((item) => !deleted.has(Number(item?.actionPlanId))); return { ...data, meta: { ...(data.meta || {}), importedActionHistoryVersion: 8 }, actionPlans: [...byId.values()], notifications }; }
