...
export class SharedStore {
  ...
  async fetch(request) {
    ...
    const liveActionDelete = path.match(/^\\/api\\/live-actions\\/(\\d+)$/); if (liveActionDelete) { if (request.method !== "DELETE") return Response.json({ error: "Método não permitido." }, { status: 405 }); const actionId = Number(liveActionDelete[1]); await this.state.storage.put("liveActions", ((await this.state.storage.get("liveActions")) || []).filter((item) => Number(item.id) !== actionId)); const deleted = new Set((await this.state.storage.get("deletedLiveActionIds")) || []); deleted.add(actionId); await this.state.storage.put("deletedLiveActionIds", [...deleted]); await this.state.storage.put("liveNotifications", ((await this.state.storage.get("liveNotifications")) || []).filter((item) => Number(item.actionPlanId) !== actionId)); const data = (await this.state.storage.get("data")) || null; if (data) { data.actionPlans = (Array.isArray(data.actionPlans) ? data.actionPlans : []).filter((item) => Number(item.id) !== actionId); data.notifications = (Array.isArray(data.notifications) ? data.notifications : []).filter((item) => Number(item.actionPlanId) !== actionId); await this.state.storage.put("data", data); } return Response.json({ success: true }); }
    if (path === "/api/live-actions") { if (request.method !== "POST") return Response.json({ error: "Método não permitido." }, { status: 405 }); const body = await request.json().catch(() => ({})); let item = body?.item; if (!item || typeof item !== "object" || !item.id) return Response.json({ error: "Ação inválida." }, { status: 400 }); const live = (await this.state.storage.get("liveActions")) || []; let existing = live.findIndex((entry) => Number(entry.id) === Number(item.id)); const sameAction = existing >= 0 && ((item.syncId && item.syncId === live[existing].syncId) || (!item.syncId && !live[existing].syncId && item.createdAt === live[existing].createdAt)); if (existing >= 0 && !sameAction) { const data = (await this.state.storage.get("data")) || {}; const lastId = Math.max(Number(data.sequence?.actionPlans || 0), ...live.map((entry) => Number(entry.id) || 0), Number(item.id) || 0); item = { ...item, id: lastId + 1 }; existing = -1; } if (existing >= 0) live[existing] = item; else live.push(item); await this.state.storage.put("liveActions", live); const deleted = new Set((await this.state.storage.get("deletedLiveActionIds")) || []); deleted.delete(Number(item.id)); await this.state.storage.put("deletedLiveActionIds", [...deleted]); const data = (await this.state.storage.get("data")) || null; if (data) { data.sequence = { ...(data.sequence || {}), actionPlans: Math.max(Number(data.sequence?.actionPlans || 0), Number(item.id) || 0) }; await this.state.storage.put("data", data); } let notifications = (await this.state.storage.get("liveNotifications")) || []; if (Number(item.ownerId) && item.status !== "done" && !notifications.some((entry) => Number(entry.actionPlanId) === Number(item.id))) notifications.push({ id: "live-" + item.id, userId: Number(item.ownerId), actionPlanId: Number(item.id), title: "Ação sob sua responsabilidade", message: item.title || item.objective || "Nova ação atribuída.", level: "info", link: "actionPlans", read: false, createdAt: new Date().toISOString() }); if (item.status === "done") notifications = notifications.filter((entry) => Number(entry.actionPlanId) !== Number(item.id)); await this.state.storage.put("liveNotifications", notifications); return Response.json({ success: true, item }); }
